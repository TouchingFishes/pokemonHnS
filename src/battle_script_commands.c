...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_KINGSROCK_SHELLBELL: // king's rock and shell bell
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_DEFENDER_ITEM: //rocky helmet, weakness policy, resist berries
            if (ItemBattleEffects(ITEMEFFECT_DEFENDER_ITEM, 0, FALSE))
                effect = TRUE;
            gBattleScripting.moveendState++;
            break;
...
static u32 GetTrainerMoneyToGive(u16 trainerId)
{
    ...
    else
    {
        ...
        case F_TRAINER_PARTY_LEVEL_SCALED:
            {
                ...
                lastMonLevel = ResolveTrainerMonLevel(party[gTrainers[trainerId].partySize - 1].lvl);
            }
            break;
        ...
        case F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_CUSTOM_ABILITY | F_TRAINER_PARTY_LEVEL_SCALED:
            {
                const struct TrainerMonNoItemCustomMovesAbilities *party = gTrainers[trainerId].party.NoItemCustomMovesAbility;
                lastMonLevel = ResolveTrainerMonLevel(party[gTrainers[trainerId].partySize - 1].lvl);
            }
            break;
        ...
        case F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM | F_TRAINER_PARTY_CUSTOM_ABILITY | F_TRAINER_PARTY_LEVEL_SCALED:
            {
                const struct TrainerMonItemCustomMovesAbilities *party = gTrainers[trainerId].party.ItemCustomMovesAbility;
                lastMonLevel = ResolveTrainerMonLevel(party[gTrainers[trainerId].partySize - 1].lvl);
            }
            break;
        }
